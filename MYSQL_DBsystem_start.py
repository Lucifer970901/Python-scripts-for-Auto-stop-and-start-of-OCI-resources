...
db_systems = db_client.list_db_systems(compartment_id,lifecycle_state='INACTIVE').data #
if len(db_systems)>0:
        ...
        if ('Dev' in db_system.freeform_tags and db_system.freeform_tags['Dev'] == 'Yes')or('Test' in db_system.freeform_tags and db_system.freeform_tags['Test'] == 'Yes'):
            if len(mysqldb.data)>0:
                for db in mysqldb.data:
                    ...
                    db_client.start_db_system(db.id)
                    print("\n")
            else:
    ...
    print("No DB systems in the inactive state in compartment")
